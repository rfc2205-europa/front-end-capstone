...
const ModalView = function({clickFunc, product_id, needChar}) {
  ...
  const submitChars = function(e) {
    ...
    if (!chars.rating || chars.body.length < 50 || chars.body.length > 1000 || chars.name.length > 60 || !validateEmail(chars.email)) {
      ...
      setInvalid(false);
      const postChars = {...chars, type: 'review', api: 'https://app-hrsei-api.herokuapp.com/api/fec2/hr-rfc/reviews'};
  ...
  return ReactDom.createPortal(
      <>
        ...
        <div className='review-addReview-modal-content'>
...
var AddReview = function({product_id, needChar}) {
  ...
  return (
    <>
      <button className="review-btn" onClick={() => {setModal(true)}}>Add Review</button>
